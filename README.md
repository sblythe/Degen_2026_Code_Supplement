...
To generate them, run the first code block of **`plot_100xSimResults.m`** to load your simulation results (produced by **`simulate_K27me_final.m`** or included in in `~/22-Mar-2026 18:40:31_mat/` and `~/22-Mar-2026 18:41:58_pat/` as discussed above). Run the third code block to calculate and plot the maternal vs. paternal contributions to H3K27 methylation.
